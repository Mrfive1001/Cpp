...
* 类内变量的作用域仅为对象本身，对于类内所需要的常量来说可以有两种方法来解决(const int a = 10不可行，因为这在类声明中定义了一个变量)
  * enum {month = 10};这只是定义了一个month的标记，在类中所有遇到month的地方都进行替换，而没有创建变量
  * static const int month = 10;所有实例共享同一个变量，且const变量需在定义时候初始化，对于非const变量初始化必须在类外?这个原因待定?
* 